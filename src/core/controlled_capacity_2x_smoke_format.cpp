...
Result<ControlledCapacity2xSmokeOptions>
parse_controlled_capacity_2x_smoke_arguments(std::span<const std::string_view> arguments) noexcept {
    ...
    for (std::size_t i = 0U; i < arguments.size(); ++i) {
        if (arguments[i] == "--run-controlled-capacity-2x-smoke" && !options.acknowledged) {
...
format_controlled_capacity_2x_smoke_report(const ControlledCapacity2xSmokeReport& report,
    ...
    writer.object_array("approved_providers", report.approved_providers,
                        [](SmokeJsonWriter& out,
                           const ControlledCapacity2xSmokeReport::Provider& provider) noexcept {
    ...
    writer.number("maximum_physical_bytes", m13_physical_cap.value());
    writer.number("budget_hard_limit", m13_physical_cap.value());
    writer.number("migration_reserve", m13_migration_reserve.value());
    writer.number("normal_admission_limit", m13_normal_limit.value());
    writer.number("maximum_transitions_per_cycle", 1U);
    writer.number("host_budget", 0U);
    writer.number("capacity_reference_bytes", m13_settled_target.value());
    writer.text({"policy_mode", "GPU_RESIDENT"});
    writer.number("pressure_high_threshold", m13_high_threshold.value());
    writer.number("pressure_settled_threshold", m13_settled_target.value());
    writer.number("maximum_policy_cycles", m13_maximum_cycles);
    writer.number("chunk_count", m13_chunk_count);
    writer.number("logical_bytes_per_chunk", m13_logical_payload.value());
    writer.number("configured_aggregate_logical_bytes", m13_aggregate_logical.value());
    ...
    writer.boolean("controlled_capacity_2_0x_proven", report.controlled_capacity_2_0x_proven);
    ...
    writer.boolean("warm_chunk_preserved_raw", report.warm_chunk_preserved_raw);
    ...
    writer.text({"hardware_validation", physical_execution && valid_source &&
                                                ...
                                                report.controlled_capacity_snapshot_proven &&
                                                report.controlled_capacity_2_0x_proven &&
                                                report.final_counts_known
                                            ? "CONTROLLED_CAPACITY_2_0X_VALIDATED"
                                            : "NOT_TESTED"});
