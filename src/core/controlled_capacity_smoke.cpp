...
RuntimeConfig controlled_capacity_configuration() noexcept {
    RuntimeConfig config{};
    config.required_capabilities.host_tier = false;
    config.preferred_chunk_size = m12_logical_payload;
    config.budgets.gpu = {m12_physical_cap, m12_settled_target, m12_migration_reserve};
    config.policy.mode = PolicyMode::gpu_resident;
    config.policy.tuning.maximum_transitions = 1U;
    return config;
}

Result<bool> controlled_capacity_1_5x(ByteSize logical, ByteSize physical) noexcept {
    const auto left = checked_mul(logical.value(), 2U, OperationId::verify);
    const auto right = checked_mul(physical.value(), 3U, OperationId::verify);
    if (!left) {
        return left.error();
    }
    if (!right) {
    ...
    return logical == m12_aggregate_logical && physical != ByteSize{} &&
           physical <= m12_settled_target && left.value() >= right.value();
}

Result<ByteSize> admit_controlled_capacity_stage(ByteSize live, ByteSize destination,
                                                 ByteSize workspace, ByteSize compaction) noexcept {
    TierUsage usage{};
    ...
    if (peak.value() > m12_physical_cap) {
...
namespace {
void fail(ControlledCapacitySmokeReport& report, Error error) noexcept {
...
[[nodiscard]] Pressure pressure(const TierUsage& usage) noexcept {
    ...
    return charged ? policy::pressure(charged.value(), m12_settled_target, m12_physical_cap,
                                      m12_migration_reserve)
                   : Pressure::critical;
}
...
// Physical construction is reachable only from the separate default-OFF M12 executable.
class CapacitySession final : public testing::PolicySelectionObserver {
  public:
    CapacitySession(Runtime runtime, CudaVmmBackend& backend, ControlledCapacitySmokeReport& report,
                    ControlledCapacityObserver* observer) noexcept
    ...
    void run() {
        std::vector<std::byte> initial(static_cast<std::size_t>(m12_logical_payload.value()));
        ...
        std::array<std::optional<Lease>, m12_chunk_count> initializing{};
        for (std::size_t index = 0U; index < buffers_.size(); ++index) {
            ...
            auto allocated = runtime_.allocate(m12_logical_payload);
            if (!accept(allocated)) {
                return;
            }
        ...
        if (!access(1U) || !access(2U) || !access(3U)) {
            return;
        }
        ...
        event(ControlledCapacityEventKind::initial_snapshot, m12_chunk_count);
        for (std::uint32_t cycle = 0U; cycle < m12_maximum_cycles; ++cycle) {
            if (policy_pressure_charge(runtime_.stats().gpu).value() <= m12_settled_target) {
                break;
            }
            if (!run_cycle(cycle)) {
                return;
        ...
        event(ControlledCapacityEventKind::policy_settled_snapshot, m12_chunk_count);
        for (std::size_t index = 0U; index < buffers_.size(); ++index) {
            fill(index, initial);
  ...
  private:
    ...
    static MemoryRange full_range() noexcept { return {{}, m12_logical_payload}; }
    static void fill(std::size_t chunk, std::span<std::byte> bytes) noexcept {
        for (std::size_t index = 0U; index < bytes.size(); ++index) {
            bytes[index] = controlled_capacity_payload_byte(chunk, index);
        }
    }
    ...
    void event(ControlledCapacityEventKind kind, std::size_t index) noexcept {
    ...
    bool corroborate(std::size_t index, ChunkSnapshot& snapshot) noexcept {
        ...
        if (allocation.value().id != authoritative.resource ||
            ...
            authoritative.metadata.logical_size != m12_logical_payload ||
    ...
    ByteSize corroborate_accounting() noexcept {
        ...
        if (!charged || !host || host.value() != ByteSize{} ||
            ...
            charged.value() > m12_physical_cap) {
            contract();
        }
    ...
    bool admit(ByteSize destination, ByteSize workspace, ByteSize compaction) noexcept {
        ...
        const auto peak = admit_controlled_capacity_stage(live, destination, workspace, compaction);
        if (!accept(peak)) {
    ...
    bool initial_snapshot() noexcept {
        ...
        for (std::size_t index = 0U; index < buffers_.size(); ++index) {
            ...
            record.effective_frequency =
                policy::frequency(record.access, runtime_.stats().policy.access_epoch,
                                  controlled_capacity_configuration().policy.tuning);
            record.temperature =
        ...
        if (report_.aggregate_raw_charge != charged || report_.initial_pressure != Pressure::hard ||
            charged <= m12_high_threshold || runtime_.stats().policy.compression_attempts != 0U ||
            ...
            report_.chunks[4].temperature != Temperature::warm ||
            report_.chunks[5].temperature != Temperature::warm) {
    ...
    bool run_cycle(std::uint32_t index) noexcept {
        ...
        cycle.selected_chunk = static_cast<std::uint32_t>(m12_chunk_count);
        ...
        event(ControlledCapacityEventKind::policy_cycle_start, index);
        // This call carries only a budget target. The production scan supplies the victim.
        const auto result =
            testing::RuntimeAccess::reclaim(runtime_, m12_settled_target, false, this);
        cycle.cycle_id = runtime_.stats().policy.policy_cycles;
        cycle.after = runtime_.stats().gpu;
        cycle.pressure_after = pressure(cycle.after);
        if (!cycle.selected || !cycle.completed) {
        ...
        if (!record.compression_completion || !record.compression_enqueue ||
            !record.compaction_exercised ||
            record.stored_crc_actual != record.stored_crc_expected || cycle.selected_chunk == 0U) {
            fail(report_, make_error(ErrorCode::integrity_failure, OperationId::verify));
            return false;
        }
        return true;
    ...
    bool settled_snapshot() noexcept {
        ...
        report_.pressure_target_reached = report_.aggregate_settled_charge <= m12_settled_target;
        ByteSize sum{};
        ByteSize logical{};
        for (std::size_t index = 0U; index < buffers_.size(); ++index) {
            ChunkSnapshot snapshot{};
        ...
        const auto capacity = controlled_capacity_1_5x(logical, sum);
        ...
        if (!capacity || !capacity.value() || !saved || sum != report_.aggregate_settled_charge ||
            ...
            report_.automatic_policy_actions != 3U || report_.cycle_count != 3U ||
            report_.cycles[0].selected_chunk != 1U || report_.cycles[1].selected_chunk != 2U ||
            report_.cycles[2].selected_chunk != 3U ||
            ...
            report_.chunks[4].settled != RepresentationState::gpu_raw ||
            report_.chunks[5].settled != RepresentationState::gpu_raw ||
    ...
    bool verify_chunk(std::size_t index, std::span<const std::byte> expected,
        ...
        event(ControlledCapacityEventKind::integrity_start, index);
        ...
        record.integrity.size_equal =
            snapshot.authoritative.metadata.logical_size == m12_logical_payload;
    ...
    void finish() noexcept {
        ...
        event(ControlledCapacityEventKind::cleanup_complete, m12_chunk_count);
        if (verified_ && !report_.has_error) {
            report_.controlled_capacity_1_5x_proven = true;
            report_.result = CompressionSmokeResult::passed;
            event(ControlledCapacityEventKind::pass_published, m12_chunk_count);
        }
    }

    Runtime runtime_;
    CudaVmmBackend& backend_;
    ControlledCapacitySmokeReport& report_;
    ControlledCapacityObserver* observer_{};
    std::array<std::optional<Buffer>, m12_chunk_count> buffers_{};
    std::array<BufferId, m12_chunk_count> identities_{};
    bool verified_{};
    bool finished_{};
};
} // namespace
...
void run_controlled_capacity_smoke(std::unique_ptr<CudaDriverApi> driver,
    ...
    report.admission.logical_size = m12_logical_payload;
    report.admission.physical_cap = m12_physical_cap;
    ...
    if (!created) {
        ...
        if (report.admission.peak > m12_physical_cap ||
    ...
    auto runtime = testing::RuntimeAccess::create_with_backend(controlled_capacity_configuration(),
