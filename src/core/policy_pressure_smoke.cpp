...
RuntimeConfig policy_pressure_configuration() noexcept {
    RuntimeConfig config{};
    config.required_capabilities.host_tier = false;
    config.preferred_chunk_size = m10_logical_payload;
    config.budgets.gpu = {m11_physical_cap, m11_settled_target, m11_migration_reserve};
    config.policy.mode = PolicyMode::gpu_resident;
    config.policy.tuning.maximum_transitions = 1U;
    return config;
}

Result<ByteSize> policy_pressure_charge(const TierUsage& usage) noexcept {
    std::uint64_t total{};
    for (const auto value :
         {usage.committed, usage.reserved, usage.staging, usage.workspace, usage.cleanup_debt}) {
        const auto sum = checked_add(total, value.value(), OperationId::verify);
        if (!sum) {
            return sum.error();
        }
        total = sum.value();
...
Result<ByteSize> admit_policy_pressure_stage(ByteSize live, ByteSize destination,
    ...
    if (peak.value() > m11_physical_cap) {
...
namespace {
void fail(PolicyPressureSmokeReport& report, Error error) noexcept {
...
[[nodiscard]] Pressure pressure(const TierUsage& usage) noexcept {
    ...
    return charged ? policy::pressure(charged.value(), m11_settled_target, m11_physical_cap,
                                      m11_migration_reserve)
                   : Pressure::critical;
}
...
// Physical construction is reachable only from the separate default-OFF M11 executable.
class PolicySession final : public testing::PolicySelectionObserver {
  public:
    PolicySession(Runtime runtime, CudaVmmBackend& backend, PolicyPressureSmokeReport& report,
                  PolicyPressureObserver* observer) noexcept
    ...
    ~PolicySession() noexcept override { finish(); }
    ...
    void run() {
        ...
        std::array<std::optional<Lease>, m10_chunk_count> initializing{};
        for (std::size_t index = 0U; index < buffers_.size(); ++index) {
            ...
            auto allocated = runtime_.allocate(m10_logical_payload);
            if (!accept(allocated)) {
                return;
            }
        ...
        event(PolicyPressureEventKind::initial_snapshot, 4U);
        for (std::uint32_t cycle = 0U; cycle < m11_maximum_cycles; ++cycle) {
            if (policy_pressure_charge(runtime_.stats().gpu).value() <= m11_settled_target) {
                break;
            }
            if (!run_cycle(cycle)) {
                return;
        ...
        event(PolicyPressureEventKind::policy_settled_snapshot, 4U);
        for (std::size_t index = 0U; index < buffers_.size(); ++index) {
            fill(index, initial);
  ...
  private:
    ...
    static MemoryRange full_range() noexcept { return {{}, m10_logical_payload}; }
    ...
    void event(PolicyPressureEventKind kind, std::size_t index) noexcept {
    ...
    bool corroborate(std::size_t index, ChunkSnapshot& snapshot) noexcept {
        ...
        if (allocation.value().id != authoritative.resource ||
            ...
            authoritative.metadata.logical_size != m10_logical_payload ||
    ...
    ByteSize corroborate_accounting() noexcept {
        ...
        if (!charged || !host || host.value() != ByteSize{} ||
            ...
            charged.value() > m11_physical_cap) {
            contract();
        }
    ...
    bool admit(ByteSize destination, ByteSize workspace, ByteSize compaction) noexcept {
        ...
        const auto peak = admit_policy_pressure_stage(live, destination, workspace, compaction);
        if (!accept(peak)) {
    ...
    bool initial_snapshot() noexcept {
        ...
        for (std::size_t index = 0U; index < buffers_.size(); ++index) {
            ...
            record.effective_frequency =
                policy::frequency(record.access, runtime_.stats().policy.access_epoch,
                                  policy_pressure_configuration().policy.tuning);
            record.temperature =
        ...
        if (report_.aggregate_raw_charge != charged || report_.initial_pressure != Pressure::hard ||
            charged <= m11_high_threshold || runtime_.stats().policy.compression_attempts != 0U ||
            ...
            report_.chunks[3].temperature != Temperature::warm) {
    ...
    bool run_cycle(std::uint32_t index) noexcept {
        ...
        event(PolicyPressureEventKind::policy_cycle_start, index);
        // This call carries only a budget target. The production scan supplies the victim.
        const auto result =
            testing::RuntimeAccess::reclaim(runtime_, m11_settled_target, false, this);
        cycle.cycle_id = runtime_.stats().policy.policy_cycles;
        cycle.after = runtime_.stats().gpu;
        cycle.pressure_after = pressure(cycle.after);
        if (!cycle.selected || !cycle.completed) {
    ...
    bool settled_snapshot() noexcept {
        ...
        report_.pressure_target_reached = report_.aggregate_settled_charge <= m11_settled_target;
        ByteSize sum{};
        for (std::size_t index = 0U; index < buffers_.size(); ++index) {
            ChunkSnapshot snapshot{};
            if (!corroborate(index, snapshot)) {
        ...
        if (!saved || sum != report_.aggregate_settled_charge || !report_.pressure_target_reached ||
            !report_.hot_chunk_preserved_raw || report_.automatic_policy_actions != 2U ||
            report_.cycle_count != 2U || report_.cycles[0].selected_chunk != 1U ||
            report_.cycles[1].selected_chunk != 2U ||
            report_.chunks[1].settled != RepresentationState::gpu_compressed ||
            report_.chunks[2].settled != RepresentationState::gpu_compressed ||
            report_.chunks[3].settled != RepresentationState::gpu_raw ||
            report_.settled_pressure != Pressure::normal) {
            fail(report_, make_error(ErrorCode::unsupported, OperationId::verify));
            return false;
        }
        report_.reclaimed = ByteSize{saved.value()};
        report_.policy_settled_snapshot_proven = true;
        return true;
    }
    bool verify_chunk(std::size_t index, std::span<const std::byte> expected,
                      std::span<std::byte> observed) noexcept {
        if (!report_.policy_settled_snapshot_proven) {
            contract();
        }
        auto& record = report_.chunks[index];
        ...
        event(PolicyPressureEventKind::integrity_start, index);
        ...
        record.integrity.size_equal =
            snapshot.authoritative.metadata.logical_size == m10_logical_payload;
    ...
    void finish() noexcept {
        ...
        event(PolicyPressureEventKind::cleanup_complete, 4U);
        if (verified_ && !report_.has_error) {
            report_.result = CompressionSmokeResult::passed;
            event(PolicyPressureEventKind::pass_published, 4U);
        }
    }

    Runtime runtime_;
    CudaVmmBackend& backend_;
    PolicyPressureSmokeReport& report_;
    PolicyPressureObserver* observer_{};
    std::array<std::optional<Buffer>, m10_chunk_count> buffers_{};
    std::array<BufferId, m10_chunk_count> identities_{};
    bool verified_{};
    bool finished_{};
};
} // namespace
...
void run_policy_pressure_smoke(std::unique_ptr<CudaDriverApi> driver,
    ...
    report.admission.physical_cap = m11_physical_cap;
    ...
    if (!created) {
        ...
        if (report.admission.peak > m11_physical_cap ||
    ...
    auto runtime = testing::RuntimeAccess::create_with_backend(policy_pressure_configuration(),
    ...
    try {
        PolicySession session{std::move(runtime).value(), *backend, report, observer};
        session.run();
