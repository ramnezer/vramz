...
Result<PolicyPressureSmokeOptions>
parse_policy_pressure_smoke_arguments(std::span<const std::string_view> arguments) noexcept {
    ...
    for (std::size_t i = 0U; i < arguments.size(); ++i) {
        if (arguments[i] == "--run-policy-pressure-smoke" && !options.acknowledged) {
...
FormatResult format_policy_pressure_smoke_report(const PolicyPressureSmokeReport& report,
                                                 std::string_view source_sha256,
                                                 bool physical_execution,
                                                 std::span<char> output) noexcept {
    SmokeJsonWriter writer{output};
    writer.number("m11_version", 1U);
    ...
    writer.object_array(
        ...
        [](SmokeJsonWriter& out, const PolicyPressureSmokeReport::Provider& provider) noexcept {
    ...
    writer.number("maximum_physical_bytes", m11_physical_cap.value());
    writer.number("budget_hard_limit", m11_physical_cap.value());
    writer.number("migration_reserve", m11_migration_reserve.value());
    writer.text({"policy_mode", "GPU_RESIDENT"});
    writer.number("pressure_high_threshold", m11_high_threshold.value());
    writer.number("pressure_settled_threshold", m11_settled_target.value());
    writer.number("maximum_policy_cycles", m11_maximum_cycles);
    writer.number("chunk_count", m10_chunk_count);
    writer.number("logical_bytes_per_chunk", m10_logical_payload.value());
    writer.number("aggregate_logical_bytes", m10_chunk_count * m10_logical_payload.value());
    ...
    writer.boolean("policy_settled_snapshot_proven", report.policy_settled_snapshot_proven);
    ...
    writer.text({"hardware_validation", physical_execution && valid_source &&
                                                ...
                                                report.policy_settled_snapshot_proven &&
                                                report.final_counts_known
                                            ? "POLICY_PRESSURE_VALIDATED"
                                            : "NOT_TESTED"});
