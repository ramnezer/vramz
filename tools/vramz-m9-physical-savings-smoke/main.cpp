...
constinit vramz::detail::CompressionSmokeReport report{};
...
[[nodiscard]] bool emit_report() noexcept {
    ...
    const auto formatted = vramz::detail::format_physical_savings_smoke_report(
        report, vramz::detail::m9_source_sha256, physical_execution,
    ...
    if (formatted.truncated || formatted.written >= output.size()) {
        constexpr std::string_view fallback{
            "{\"m9_version\":1,\"result\":\"FAIL\",\"error\":\"report_format_failure\","
...
[[noreturn]] void fatal_termination() noexcept {
    vramz::detail::finalize_compression_smoke_failure(report);
...
loaded_file_is_approved(const void* symbol,
                        const vramz::detail::M9LibraryIdentity& approved) noexcept {
    Dl_info loaded{};
...
[[nodiscard]] bool build_and_loader_approved() noexcept {
#if defined(VRAMZ_M9_PHYSICAL_SAVINGS_ONLY) && VRAMZ_M9_PHYSICAL_SAVINGS_ONLY == 1
    constexpr bool enabled = true;
#else
    constexpr bool enabled = false;
#endif
    const auto source = vramz::detail::m9_source_sha256;
    ...
    if (!capabilities.cuda_adapter_compiled || !capabilities.nvcomp_adapter_compiled ||
        ...
        path == nullptr || std::string_view{path} != vramz::detail::m9_loader_path) {
        return false;
    }
    const auto inspect_loaded = [](dl_phdr_info* entry, std::size_t, void*) noexcept -> int {
        ...
        for (const auto& allowed :
             {vramz::detail::m9_driver, vramz::detail::m9_nvcomp, vramz::detail::m9_cudart}) {
...
installed_driver_release() noexcept {
    ...
    if (!parsed || parsed.value() != vramz::detail::m9_expected_driver_release) {
        return invalid;
    }
    return parsed.value();
}
...
int main(int argc, char* argv[]) {
    ...
    const auto parsed =
        vramz::detail::parse_physical_savings_smoke_arguments(std::span{arguments}.first(count));
    if (!parsed) {
        return reject(parsed.error());
    }
    auto options = parsed.value();
    report.device_ordinal = options.device;
    report.admission.logical_size = vramz::detail::m9_logical_payload;
    report.admission.physical_cap = vramz::detail::m9_physical_cap;
    ...
    options.prior_driver_api = vramz::detail::m9_prior_driver_api;
    ...
    vramz::detail::run_physical_savings_smoke(std::move(driver).value(), std::move(codec).value(),
                                              options, report);
    if (!emit_report()) {
        return 1;
    }
