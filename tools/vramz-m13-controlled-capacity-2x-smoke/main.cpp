...
constinit vramz::detail::ControlledCapacity2xSmokeReport report{};
...
[[nodiscard]] bool emit_report() noexcept {
    report.approved_providers = {
        {{"driver", vramz::detail::m13_driver.path, vramz::detail::m13_driver.sha256},
         {"nvcomp", vramz::detail::m13_nvcomp.path, vramz::detail::m13_nvcomp.sha256},
         {"cudart", vramz::detail::m13_cudart.path, vramz::detail::m13_cudart.sha256}}};
    std::array<char, 65536U> output{};
    const auto formatted = vramz::detail::format_controlled_capacity_2x_smoke_report(
        report, vramz::detail::m13_source_sha256, physical_execution,
    ...
    if (formatted.truncated || formatted.written >= output.size()) {
        constexpr std::string_view fallback{
            "{\"m13_version\":1,\"result\":\"FAIL\",\"error\":\"report_format_failure\","
...
[[noreturn]] void fatal_termination() noexcept {
    vramz::detail::finalize_controlled_capacity_2x_smoke_failure(report);
...
loaded_file_is_approved(const void* symbol,
                        const vramz::detail::M13LibraryIdentity& approved) noexcept {
    Dl_info loaded{};
...
[[nodiscard]] bool build_and_loader_approved() noexcept {
#if defined(VRAMZ_M13_CONTROLLED_CAPACITY_2X_ONLY) && VRAMZ_M13_CONTROLLED_CAPACITY_2X_ONLY == 1
    constexpr bool enabled = true;
#else
    constexpr bool enabled = false;
#endif
    const auto source = vramz::detail::m13_source_sha256;
    ...
    if (!capabilities.cuda_adapter_compiled || !capabilities.nvcomp_adapter_compiled ||
        ...
        path == nullptr || std::string_view{path} != vramz::detail::m13_loader_path) {
        return false;
    }
    const auto inspect_loaded = [](dl_phdr_info* entry, std::size_t, void*) noexcept -> int {
        ...
        for (const auto& allowed :
             {vramz::detail::m13_driver, vramz::detail::m13_nvcomp, vramz::detail::m13_cudart}) {
...
installed_driver_release() noexcept {
    ...
    if (!parsed || parsed.value() != vramz::detail::m13_expected_driver_release) {
        return invalid;
    }
    return parsed.value();
}
...
int main(int argc, char* argv[]) {
    ...
    const auto parsed = vramz::detail::parse_controlled_capacity_2x_smoke_arguments(
        std::span{arguments}.first(count));
    if (!parsed) {
    ...
    options.prior_driver_api = vramz::detail::m13_prior_driver_api;
    ...
    vramz::detail::run_controlled_capacity_2x_smoke(std::move(driver).value(),
