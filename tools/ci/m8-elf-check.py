#!/usr/bin/env python3
"""Read M8 ELF closure and hashes; never load/execute inspected ELF files."""
...
PINNED = {
    "libcuda.so.1": ("/usr/lib/x86_64-linux-gnu/libcuda.so.595.84",
...
def inspect(path):
    ...
    m7.require("stubs" not in path.parts, "stub in M8 runtime closure")
    before = path.stat()
...
def closure(driver, cuda, nvcomp, binary=None):
    ...
    m7.require(all(r.is_absolute() and not str(r).startswith("/usr/local/") for r in roots),
               "M8 requires isolated development roots")
    ...
    if binary is not None:
        ...
        m7.require(set(PINNED).issubset(executable["needed"]), "M8 binary missing a direct NVIDIA dependency")
...
def identity_config(report):
    ...
    for kind, name in (("DRIVER", "libcuda.so.1"), ("NVCOMP", "libnvcomp.so.5"), ("CUDART", "libcudart.so.13")):
        ...
        lines.append(f"set(VRAMZ_M8_{kind}_IDENTITY [=[{value}]=])")
        lines.append(f"set(VRAMZ_M8_{kind}_PATH [=[{item['path']}]=])")
    lines.append(f"set(VRAMZ_M8_LOADER_PATH [=[{report['loader_path']}]=])")
    lines.append("set(VRAMZ_M8_DRIVER_RELEASE [=[" + ", ".join(str(x) + "U" for x in report["nvidia_driver_release"]) + "]=])")
    lines.append(f"set(VRAMZ_M8_PRIOR_DRIVER_API {PRIOR_M7_DRIVER_API})")
    return "\n".join(lines) + "\n"
