...
[[nodiscard]] bool emit_report() noexcept {
    ...
    const auto formatted = vramz::detail::format_compression_smoke_report(
        report, vramz::detail::m8_source_sha256, physical_execution,
    ...
    if (formatted.truncated || formatted.written >= output.size()) {
        constexpr std::string_view fallback{
            "{\"m8_version\":1,\"result\":\"FAIL\",\"error\":\"report_format_failure\","
...
loaded_file_is_approved(const void* symbol,
                        const vramz::detail::M8LibraryIdentity& approved) noexcept {
    Dl_info loaded{};
...
[[nodiscard]] bool build_and_loader_approved() noexcept {
#if defined(VRAMZ_M8_COMPRESSION_ONLY) && VRAMZ_M8_COMPRESSION_ONLY == 1
    constexpr bool enabled = true;
#else
    constexpr bool enabled = false;
#endif
    const auto source = vramz::detail::m8_source_sha256;
    ...
    if (!capabilities.cuda_adapter_compiled || !capabilities.nvcomp_adapter_compiled ||
        ...
        path == nullptr || std::string_view{path} != vramz::detail::m8_loader_path) {
        return false;
    }
    const auto inspect_loaded = [](dl_phdr_info* entry, std::size_t, void*) noexcept -> int {
        ...
        for (const auto& allowed :
             {vramz::detail::m8_driver, vramz::detail::m8_nvcomp, vramz::detail::m8_cudart}) {
...
installed_driver_release() noexcept {
    ...
    if (!parsed || parsed.value() != vramz::detail::m8_expected_driver_release) {
        return invalid;
    }
    return parsed.value();
}
...
int main(int argc, char* argv[]) {
    ...
    const auto parsed =
        vramz::detail::parse_compression_smoke_arguments(std::span{arguments}.first(count));
    if (!parsed) {
    ...
    options.prior_driver_api = vramz::detail::m8_prior_driver_api;
    ...
    vramz::detail::run_compression_smoke(std::move(driver).value(), std::move(codec).value(),
                                         options, report);
    if (!emit_report()) {
        return 1;
    }
