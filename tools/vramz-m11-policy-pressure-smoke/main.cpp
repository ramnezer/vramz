...
constinit vramz::detail::PolicyPressureSmokeReport report{};
...
[[nodiscard]] bool emit_report() noexcept {
    report.approved_providers = {
        {{"driver", vramz::detail::m11_driver.path, vramz::detail::m11_driver.sha256},
         {"nvcomp", vramz::detail::m11_nvcomp.path, vramz::detail::m11_nvcomp.sha256},
         {"cudart", vramz::detail::m11_cudart.path, vramz::detail::m11_cudart.sha256}}};
    std::array<char, 32768U> output{};
    const auto formatted = vramz::detail::format_policy_pressure_smoke_report(
        report, vramz::detail::m11_source_sha256, physical_execution,
    ...
    if (formatted.truncated || formatted.written >= output.size()) {
        constexpr std::string_view fallback{
            "{\"m11_version\":1,\"result\":\"FAIL\",\"error\":\"report_format_failure\","
...
[[noreturn]] void fatal_termination() noexcept {
    vramz::detail::finalize_policy_pressure_smoke_failure(report);
...
loaded_file_is_approved(const void* symbol,
                        const vramz::detail::M11LibraryIdentity& approved) noexcept {
    Dl_info loaded{};
...
[[nodiscard]] bool build_and_loader_approved() noexcept {
#if defined(VRAMZ_M11_POLICY_PRESSURE_ONLY) && VRAMZ_M11_POLICY_PRESSURE_ONLY == 1
    constexpr bool enabled = true;
#else
    constexpr bool enabled = false;
#endif
    const auto source = vramz::detail::m11_source_sha256;
    ...
    if (!capabilities.cuda_adapter_compiled || !capabilities.nvcomp_adapter_compiled ||
        ...
        path == nullptr || std::string_view{path} != vramz::detail::m11_loader_path) {
        return false;
    }
    const auto inspect_loaded = [](dl_phdr_info* entry, std::size_t, void*) noexcept -> int {
        ...
        for (const auto& allowed :
             {vramz::detail::m11_driver, vramz::detail::m11_nvcomp, vramz::detail::m11_cudart}) {
...
installed_driver_release() noexcept {
    ...
    if (!parsed || parsed.value() != vramz::detail::m11_expected_driver_release) {
        return invalid;
    }
    return parsed.value();
}
...
int main(int argc, char* argv[]) {
    ...
    const auto parsed =
        vramz::detail::parse_policy_pressure_smoke_arguments(std::span{arguments}.first(count));
    if (!parsed) {
    ...
    options.prior_driver_api = vramz::detail::m11_prior_driver_api;
    ...
    vramz::detail::run_policy_pressure_smoke(std::move(driver).value(), std::move(codec).value(),
                                             options, report);
    if (!emit_report()) {
        return 1;
    }
