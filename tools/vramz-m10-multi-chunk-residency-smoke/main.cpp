...
constinit vramz::detail::MultiChunkResidencySmokeReport report{};
...
[[nodiscard]] bool emit_report() noexcept {
    std::array<char, 16384U> output{};
    const auto formatted = vramz::detail::format_multi_chunk_residency_smoke_report(
        report, vramz::detail::m10_source_sha256, physical_execution,
    ...
    if (formatted.truncated || formatted.written >= output.size()) {
        constexpr std::string_view fallback{
            "{\"m10_version\":1,\"result\":\"FAIL\",\"error\":\"report_format_failure\","
...
[[noreturn]] void fatal_termination() noexcept {
    vramz::detail::finalize_multi_chunk_residency_smoke_failure(report);
...
loaded_file_is_approved(const void* symbol,
                        const vramz::detail::M10LibraryIdentity& approved) noexcept {
    Dl_info loaded{};
...
[[nodiscard]] bool build_and_loader_approved() noexcept {
#if defined(VRAMZ_M10_MULTI_CHUNK_RESIDENCY_ONLY) && VRAMZ_M10_MULTI_CHUNK_RESIDENCY_ONLY == 1
    constexpr bool enabled = true;
#else
    constexpr bool enabled = false;
#endif
    const auto source = vramz::detail::m10_source_sha256;
    ...
    if (!capabilities.cuda_adapter_compiled || !capabilities.nvcomp_adapter_compiled ||
        ...
        path == nullptr || std::string_view{path} != vramz::detail::m10_loader_path) {
        return false;
    }
    const auto inspect_loaded = [](dl_phdr_info* entry, std::size_t, void*) noexcept -> int {
        ...
        for (const auto& allowed :
             {vramz::detail::m10_driver, vramz::detail::m10_nvcomp, vramz::detail::m10_cudart}) {
...
installed_driver_release() noexcept {
    ...
    if (!parsed || parsed.value() != vramz::detail::m10_expected_driver_release) {
        return invalid;
    }
    return parsed.value();
}
...
int main(int argc, char* argv[]) {
    ...
    const auto parsed = vramz::detail::parse_multi_chunk_residency_smoke_arguments(
    ...
    report.maximum_physical_bytes = vramz::detail::m10_physical_cap;
    ...
    options.prior_driver_api = vramz::detail::m10_prior_driver_api;
    ...
    vramz::detail::run_multi_chunk_residency_smoke(std::move(driver).value(),
