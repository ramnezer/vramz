...
struct Fixture final {
    ...
    Fixture() {
        ...
        cuda->config.physical_limit = detail::m12_physical_cap;
        cuda->config.maximum_resource = detail::m12_physical_cap;
        cuda->config.device_info.name = std::array<char, 256U>{"NVIDIA GeForce RTX 3060"};
        codec->cuda_audit = cuda;
    }
    void run(detail::ControlledCapacityObserver* observer = nullptr) {
        detail::run_controlled_capacity_smoke(std::move(driver), std::move(api),
...
class Audit final : public detail::ControlledCapacityObserver {
  public:
    void observe(detail::ControlledCapacityEvent event) noexcept override {
    ...
    std::array<detail::ControlledCapacityEvent, 16U> events{};
    std::size_t count{};
    bool overflow{};
};
void baseline(test::Runner& runner) {
    runner.begin("M12 six M10-family chunks, automatic production policy victims, HOT protection, "
    ...
    if (r.has_error) {
        std::cerr << "M12 error=" << static_cast<unsigned>(r.error.code)
    ...
    VRAMZ_CHECK(runner, r.initial_snapshot_proven && r.controlled_capacity_snapshot_proven &&
                            r.controlled_capacity_1_5x_proven &&
                            r.aggregate_logical_bytes == detail::m12_aggregate_logical);
    VRAMZ_CHECK(runner, r.aggregate_raw_charge == ByteSize{48U * mib});
    VRAMZ_CHECK(runner, r.aggregate_settled_charge == ByteSize{30U * mib} &&
                            r.reclaimed == ByteSize{18U * mib});
    VRAMZ_CHECK(runner,
    ...
    VRAMZ_CHECK(runner, r.cycle_count == 3U && r.automatic_policy_actions == 3U);
    VRAMZ_CHECK(runner, r.cycles[0].selected_chunk == 1U && r.cycles[1].selected_chunk == 2U &&
                            r.cycles[2].selected_chunk == 3U);
    const std::array crcs{383231952U, 846517591U,  3954088913U,
                          243520057U, 2323699231U, 1116507104U};
    const std::array epochs{36U, 13U, 14U, 15U, 32U, 33U};
    const std::array revisions{20U, 2U, 2U, 2U, 2U, 2U};
    ...
    VRAMZ_CHECK(runner, !audit.overflow && audit.count == 13U);
    VRAMZ_CHECK(runner, audit.events[4].kind ==
                            detail::ControlledCapacityEventKind::policy_settled_snapshot);
    VRAMZ_CHECK(runner,
                audit.events[5].kind == detail::ControlledCapacityEventKind::integrity_start);
    VRAMZ_CHECK(runner,
                audit.events[11].kind == detail::ControlledCapacityEventKind::cleanup_complete);
    VRAMZ_CHECK(runner,
                audit.events[12].kind == detail::ControlledCapacityEventKind::pass_published);
    VRAMZ_CHECK(runner, fixture.codec->observation.compressions == 3U &&
                            fixture.codec->observation.decompressions == 3U);
    ...
    VRAMZ_CHECK(runner, fixture.codec->observation.log_size == 24U);
    for (std::size_t operation = 0U; operation < 6U; ++operation) {
        VRAMZ_CHECK(runner, std::ranges::equal(std::span{fixture.codec->observation.log}.subspan(
                                                   operation * 4U, 4U),
                                               expected_order));
    }
    VRAMZ_CHECK(runner, fixture.codec->observation.metadata_publications == 6U &&
                            fixture.codec->observation.metadata_consumptions == 6U);
    ...
    for (std::size_t index = 0U; index < 6U; ++index) {
        VRAMZ_CHECK(runner, audit.events[index + 5U].kind ==
                                    detail::ControlledCapacityEventKind::integrity_start &&
                                audit.events[index + 5U].index == index);
    }
    for (std::size_t index = 0U; index < 3U; ++index) {
        ...
        VRAMZ_CHECK(runner, detail::policy_pressure_charge(cycle.before).value() ==
                                ByteSize{(48U - 6U * index) * mib});
        VRAMZ_CHECK(runner, detail::policy_pressure_charge(cycle.after).value() ==
                                ByteSize{(42U - 6U * index) * mib});
    }
    fixture.empty(runner);
    std::array<char, 65536U> json{};
    const auto formatted = detail::format_controlled_capacity_smoke_report(r, "", false, json);
    VRAMZ_CHECK(runner, !formatted.truncated);
    std::cout << "M12_FAKE_RESULT " << std::string_view{json.data(), formatted.written} << '\n';
}
class Inject final : public detail::ControlledCapacityObserver {
  public:
    Inject(Fixture& fixture, detail::ControlledCapacityEventKind when, std::uint32_t index,
    ...
    void observe(detail::ControlledCapacityEvent event) noexcept override {
        ...
        if (event.kind == detail::ControlledCapacityEventKind::policy_cycle_start &&
            nonbeneficial) {
            codec_.inject(test::GpuCodecFault::literal_only_output);
        }
        if (event.kind == detail::ControlledCapacityEventKind::initial_snapshot) {
            creates_before =
  ...
  private:
    ...
    detail::ControlledCapacityEventKind when_;
    std::uint32_t index_{};
    test::CudaCall call_{};
...
void faults(test::Runner& runner) {
    runner.begin("M12 exact 1.5x live physical boundary remains recoverable and cleans to zero");
    {
        Fixture f;
        f.codec->config.literal_prefix_bytes = ByteSize{2U * mib};
        ...
        VRAMZ_CHECK(runner, f.report.aggregate_settled_charge == detail::m12_settled_target);
        VRAMZ_CHECK(runner, f.report.chunks[1].integrity.compressed_charge == ByteSize{4U * mib});
        VRAMZ_CHECK(runner, f.report.controlled_capacity_1_5x_proven &&
                                f.report.automatic_policy_actions == 3U);
        f.empty(runner);
    }

    for (const auto occurrence : {1U, 2U, 3U}) {
        runner.begin("M12 second policy transition preparation/workspace/compaction allocation "
                     "failure cleans all retained ownership");
        Fixture f;
        Inject injection{f,
                         detail::ControlledCapacityEventKind::policy_cycle_start,
                         1U,
    ...
    for (const auto occurrence : {1U, 2U, 3U, 4U}) {
        runner.begin("M12 failed first/second/third automatic compression or restoration stops "
                     "without retry");
        Fixture f;
    ...
    for (const auto fault : {test::GpuCodecFault::output_corruption,
        ...
        runner.begin("M12 logical CRC or CRC-neutral byte mismatch during production restoration "
                     "forbids PASS");
        Fixture f;
        f.api->inject(fault, 4U);
        f.run();
        VRAMZ_CHECK(runner, f.report.result != detail::CompressionSmokeResult::passed);
        VRAMZ_CHECK(runner, f.report.controlled_capacity_snapshot_proven && f.report.has_error &&
                                !f.report.controlled_capacity_1_5x_proven);
        f.empty(runner);
    }
    runner.begin(
        "M12 stored compressed CRC detects corrupted compaction copy before policy success");
    {
        Fixture f;
        Inject injection{f,
                         detail::ControlledCapacityEventKind::policy_cycle_start,
    ...
    runner.begin("M12 nonbeneficial policy result backs off that victim and selects a different "
    ...
    runner.begin(
        "M12 smaller stored bytes without physical-unit savings cannot pass policy acceptance");
    {
        Fixture f;
    ...
    runner.begin("M12 six nonbeneficial victims exhaust the fixed cycle bound without policy "
                 "convergence or retry");
    {
        Fixture f;
        Inject injection{f,
                         detail::ControlledCapacityEventKind::pass_published,
                         0U,
                         test::CudaCall::count,
                         test::CudaFaultMode::before,
                         1U};
        ...
        f.run(&injection);
        VRAMZ_CHECK(runner, f.report.result != detail::CompressionSmokeResult::passed);
        VRAMZ_CHECK(runner, f.report.cycle_count == detail::m12_maximum_cycles);
        VRAMZ_CHECK(runner,
                    !f.report.pressure_target_reached && f.report.automatic_policy_actions == 0U);
        VRAMZ_CHECK(runner, f.codec->observation.compressions == 6U &&
                                f.codec->observation.decompressions == 0U);
    ...
    runner.begin("M12 full retained RAW plus bound/workspace/compaction peak rejects above 128 MiB "
                 "before policy mutation");
    {
        ...
        VRAMZ_CHECK(runner, f.cuda->counts[static_cast<std::size_t>(test::CudaCall::create)] == 6U);
        f.empty(runner);
    }
    runner.begin(
        "M12 impossible preflight workspace cap rejects before retained context or backing");
    {
        Fixture f;
        f.codec->config.compression_temp = detail::m12_physical_cap;
        f.run();
        VRAMZ_CHECK(runner, f.report.result == detail::CompressionSmokeResult::resource_cap);
        VRAMZ_CHECK(runner,
...
void fatal_case(test::Runner& runner, test::GpuCodecFault codec_fault, test::CudaCall call,
                ...
                detail::ControlledCapacityEvent event = {
                    detail::ControlledCapacityEventKind::policy_cycle_start, 0U}) {
...
void fatal_faults(test::Runner& runner) {
    for (const auto fault :
        ...
        runner.begin("M12 ambiguous automatic transition completion is fail-stop with no "
                     "destructor cleanup");
    ...
    for (const auto mode : {test::CudaFaultMode::before, test::CudaFaultMode::after}) {
        runner.begin("M12 definite or ambiguous compaction cleanup failure retains accounting and "
                     "never retries");
        fatal_case(runner, test::GpuCodecFault::count, test::CudaCall::unmap, mode);
    }
    for (const auto index : {0U, 1U, 2U, 3U, 4U, 5U}) {
        runner.begin("M12 each settled RAW/COMPRESSED owner retains exact debt on cleanup failure, "
                     "with no retry");
        fatal_case(runner, test::GpuCodecFault::count, test::CudaCall::unmap,
                   test::CudaFaultMode::before,
                   {detail::ControlledCapacityEventKind::integrity_start, index});
    }
}
...
template <std::size_t Count = 2U> struct PolicyModel final {
    ...
    PolicyModel() {
        ...
        auto config = detail::controlled_capacity_configuration();
        ...
        if constexpr (Count == 6U) {
            // Scale byte units only for the pinned-candidate policy fault model.
            // The full M12 fixture always uses six 8 MiB chunks and the fixed cap.
            config.budgets.gpu = {ByteSize{65536U}, ByteSize{16384U}, ByteSize{40960U}};
        ...
        for (std::size_t index = 0U; index < buffers.size(); ++index) {
            ...
            for (std::size_t i = 0U; i < data.size(); ++i) {
                data[i] = detail::controlled_capacity_payload_byte(index, i);
            }
...
void policy_models(test::Runner& runner) {
    runner.begin("M12 no-pressure production cycle performs no action or codec launch");
    {
        PolicyModel model;
    ...
    runner.begin("M12 real access invalidates revision, rejects atomically, excludes same-cycle "
    ...
    runner.begin("M12 no useful action while all candidates are HOT gives a bounded non-PASS");
    {
        PolicyModel model;
    ...
    runner.begin("M12 third cold candidate unavailable: production policy cannot reach target or "
                 "substitute a pinned owner");
    {
        PolicyModel<6U> model;
        ...
        VRAMZ_CHECK(runner, touch(4U) && touch(5U));
        for (std::uint32_t i = 0U; i < 3U; ++i) {
            VRAMZ_CHECK(runner, touch(0U));
        }
        std::array<std::optional<Lease>, 4U> pins{};
        const std::array indices{0U, 3U, 4U, 5U};
        ...
        for (std::uint32_t i = 0U; i < 2U; ++i) {
            const auto result = model.runtime().reclaim_to_target(ByteSize{16384U});
        ...
        VRAMZ_CHECK(runner, charge > ByteSize{16384U});
        VRAMZ_CHECK(runner, !model.runtime().reclaim_to_target(ByteSize{16384U}));
        VRAMZ_CHECK(runner, model.runtime().stats().policy.compression_successes == 2U);
        VRAMZ_CHECK(runner,
    ...
    runner.begin("M12 migration reserve is inside hard limit and cannot fund unaccounted emergency "
                 "headroom");
    {
        const auto config = detail::controlled_capacity_configuration();
        BudgetLedger ledger{config.budgets};
        ReservationRequest initial{PhysicalTier::gpu, ChargeBucket::staging, ByteSize{48U * mib},
        ...
        ReservationRequest migration{PhysicalTier::gpu, ChargeBucket::workspace,
                                     ByteSize{80U * mib}, AdmissionKind::migration_temporary};
        ...
        VRAMZ_CHECK(runner, ledger.charged(PhysicalTier::gpu) == detail::m12_physical_cap);
        VRAMZ_CHECK(runner, ledger.release_reservation(token));
    ...
    runner.begin("M12 ledger materialization contradiction preserves adopted ownership and blocks "
