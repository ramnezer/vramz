#!/usr/bin/env python3
"""M12 configuration and static ELF rejection tests; no inspected executable is run."""
...
class Gates(unittest.TestCase):
    def configure(self, values):
        with tempfile.TemporaryDirectory(prefix="vramz-m12-gates-") as name:
            root = Path(name)
            (root / "CMakeLists.txt").write_text(
                'cmake_minimum_required(VERSION 3.25)\nproject(M12Gates NONE)\n'
                f'include("{SOURCE}/cmake/M7RawSmoke.cmake")\n'
                f'include("{SOURCE}/cmake/M8CompressionSmoke.cmake")\n'
                f'include("{SOURCE}/cmake/M12ControlledCapacitySmoke.cmake")\n'
                'vramz_add_m12_smoke()\n'
                'if(TARGET vramz-m12-controlled-capacity-smoke)\n'
    ...
    def test_m7_and_m12_are_exclusive(self):
        status, text = self.configure({"VRAMZ_BUILD_M7_RAW_SMOKE": "ON", "VRAMZ_BUILD_M12_CONTROLLED_CAPACITY_SMOKE": "ON"})
        self.assertNotEqual(status, 0)
        self.assertIn("mutually exclusive", text)

    def physical_values(self):
        return {**{key: "ON" for key in ("VRAMZ_BUILD_M12_CONTROLLED_CAPACITY_SMOKE", "VRAMZ_ENABLE_CUDA",
    ...
    def test_m7_obsolete_or_invalid_hash_never_identifies_m12(self):
        for digest in ("", "a" * 63, "0" * 64, "A" * 64,
            ...
            with self.subTest(digest=digest):
                status, text = self.configure({**self.physical_values(), "VRAMZ_M12_SOURCE_SHA256": digest})
                self.assertNotEqual(status, 0)
    ...
    def test_unreviewed_or_shared_lz4_is_rejected_before_dependency_setup(self):
        for library in ("", "/usr/lib/x86_64-linux-gnu/liblz4.so", "/tmp/liblz4.a"):
            with self.subTest(library=library):
                status, text = self.configure({**self.physical_values(),
                    "VRAMZ_M12_SOURCE_SHA256": "a" * 64, "VRAMZ_LZ4_LIBRARY": library,
    ...
    def test_gate_excludes_m8(self):
        status, text = self.configure({"VRAMZ_BUILD_M8_COMPRESSION_SMOKE": "ON", "VRAMZ_BUILD_M12_CONTROLLED_CAPACITY_SMOKE": "ON"})
        self.assertNotEqual(status, 0)
        self.assertIn("mutually exclusive", text)

    def test_gate_excludes_m9(self):
        status, text = self.configure({"VRAMZ_BUILD_M9_PHYSICAL_SAVINGS_SMOKE": "ON", "VRAMZ_BUILD_M12_CONTROLLED_CAPACITY_SMOKE": "ON"})
        self.assertNotEqual(status, 0)
        self.assertIn("mutually exclusive", text)

    def test_gate_excludes_m10(self):
        status, text = self.configure({"VRAMZ_BUILD_M10_MULTI_CHUNK_RESIDENCY_SMOKE": "ON", "VRAMZ_BUILD_M12_CONTROLLED_CAPACITY_SMOKE": "ON"})
        self.assertNotEqual(status, 0)
        self.assertIn("mutually exclusive", text)

    def test_gate_excludes_m11(self):
        status, text = self.configure({"VRAMZ_BUILD_M11_POLICY_PRESSURE_SMOKE": "ON", "VRAMZ_BUILD_M12_CONTROLLED_CAPACITY_SMOKE": "ON"})
        self.assertNotEqual(status, 0)
        self.assertIn("mutually exclusive", text)

    def test_physical_main_is_object_only_in_quality_builds(self):
        module = (SOURCE / "cmake/M12ControlledCapacitySmoke.cmake").read_text()
        ...
        main = (SOURCE / "tools/vramz-m12-controlled-capacity-smoke/main.cpp").read_text()
        self.assertIn("VRAMZ_M12_CONTROLLED_CAPACITY_ONLY", main)
    ...
    def test_harness_uses_production_policy_without_victim_or_transition_override(self):
        harness = (SOURCE / "src/core/controlled_capacity_smoke.cpp").read_text()
        self.assertIn("RuntimeAccess::reclaim(runtime_, m12_settled_target, false, this)", harness)
    ...
    def test_physical_main_has_no_test_observer_or_runtime_selection_override(self):
        main = (SOURCE / "tools/vramz-m12-controlled-capacity-smoke/main.cpp").read_text()
        import re
        calls = re.findall(r"run_controlled_capacity_smoke\((.*?)\);", main, re.S)
    ...
    def test_prehardware_explicitly_rejects_m12(self):
        script = (SOURCE / "tools/vramz-prehardware-check.py").read_text()
        self.assertIn("VRAMZ_BUILD_M12_CONTROLLED_CAPACITY_SMOKE", script)
        self.assertIn("vramz-m12-controlled-capacity-smoke", script)
